...
class TrustedFirstParty:
    ...
    def generate_additive_triple(size0, size1, op, device=None, *args, **kwargs):
        ...
        a = torch.full(size0, 1)
        b = torch.full(size1, 1)
        # c = getattr(torch, op)(a, b, *args, **kwargs)
        if op=="matmul":
            if(len(size0)==2):
                size2 = [size0[0], size1[-1]]
            elif(len(size0)==3):
                size2 = [size0[0], size0[1], size1[-1]]
            else:
                size2 = [size0[0], size0[1], size0[2], size1[-1]]
            c = torch.full(size2, size0[-1])
        else:
            c = torch.full(size0, 1)
